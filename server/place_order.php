...
if(isset($_POST['place_order'])){
    //1/getting user info and storing it in DB
     $name = $_POST['name'];
     $email = $_POST['email'];
     $phone = $_POST['phone'];
     ...
     $order_status = "on_hold"; //<-- Placed but not paid order
     $user_id = 1;
     $order_date = date("Y-m-d H:i:s");

    $stmt = $conn->prepare("INSERT INTO orders (order_cost,
                    order_status, user_id, user_phone, 
    ...
    //2.get products from cart(from $_SESSION)
    ...
    header('location: ../payment.php?order_status=Order Placed Successfully! You will recieve and e-mail regarding the delivery date');
